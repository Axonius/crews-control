...
_TOOLS_MAP: dict[str, Callable] = {
    'serper': lambda **kwargs: SerperDevTool(**kwargs),
    'website_search': lambda **kwargs: WebsiteContentQueryTool(**kwargs),
    'human': lambda: HumanTool(),
    'read_file': lambda: load_tools(['read_file'])[0],
    'directory_search': lambda **kwargs: DirectorySearchTool(**kwargs),
    'jql_query': lambda **kwargs: crewai_jira_tool(**kwargs),
    'selenium': lambda **kwargs: SeleniumScrapingTool(**kwargs),
...
def get_tool(tool_name: str, task_id: typing.Optional[str] = None, **kwargs) -> Callable:
    try:
        if tool_name in tools_requiring_app:
                ...
                return _TOOLS_MAP[tool_name](app=app, **kwargs)
        else:
